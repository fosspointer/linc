...
namespace linc
{
    ...
    std::unique_ptr<const Expression> TypeExpression::clone() const
    {
        ...
        if(auto function_root = std::get_if<FunctionRoot>(&m_root))
        {
            ...
            for(const auto& argument: function_root->argumentTypes)
            {
                auto clone = Types::uniqueCast<const TypeExpression>(argument.type->clone());
                arguments.push_back(DelimitedType{std::move(clone), argument.delimeter});
            }
