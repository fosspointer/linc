...
namespace linc
{
    BoundWhileExpression::BoundWhileExpression(Types::type type, std::unique_ptr<const BoundExpression> test_expression,
        ...
        :BoundExpression(type), m_testExpression(std::move(test_expression)), m_whileBody(std::move(while_body)), m_finallyBody(std::move(finally_body)),
    ...
    std::unique_ptr<const BoundExpression> BoundWhileExpression::clone() const
    {
        ...
        return std::make_unique<const BoundWhileExpression>(getType(), std::move(test_expression), std::move(while_body), std::move(finally_body),
            std::move(else_body));
    }
