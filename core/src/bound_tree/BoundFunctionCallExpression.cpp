...
namespace linc
{
    ...
    std::unique_ptr<const BoundExpression> BoundFunctionCallExpression::clone_const() const
    {
        std::vector<Argument> arguments;
        for(const auto& argument: m_arguments)
            arguments.push_back(Argument{
                .name = argument.name,
                .value = std::move(argument.value->clone_const()),
                .isMutable = argument.isMutable
            });

        return std::make_unique<const BoundFunctionCallExpression>(getType(), m_name, std::move(arguments), std::move(m_body->clone_const()));
    }
