...
namespace linc
{
    ...
    {
    public:
        BoundWhileExpression(Types::type type, std::unique_ptr<const BoundExpression> test_expression, std::unique_ptr<const BoundExpression> while_body,
            std::unique_ptr<const BoundExpression> finally_body, std::unique_ptr<const BoundExpression> else_body);
