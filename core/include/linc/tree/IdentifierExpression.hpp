...
{
    class GenericClause;
    class NamespaceClause;
    class IdentifierExpression final : public Expression 
    {
    public:
        IdentifierExpression(const Token& token, std::unique_ptr<const NamespaceClause> _namespace, std::unique_ptr<const GenericClause> generic);
        ...
        [[nodiscard]] const NamespaceClause* const getNamespaceClause() const { return m_namespace? m_namespace.get(): nullptr; }
        [[nodiscard]] const GenericClause* const getGeneric() const { return m_generic? m_generic.get(): nullptr; }
    private:
        const Token m_identifierToken;
        const std::unique_ptr<const NamespaceClause> m_namespace;
